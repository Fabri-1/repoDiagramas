...
export class Mapa2Page implements OnInit {
  ...
  async crearMapa(){
    ...
    if(this.mapRef){
      this.map2 = document.getElementById('map');
      this.map2 = new google.maps.Map(this.mapRef.nativeElement, {
        zoom: 10,
        center: myLatLng,
        mapTypeId: google.maps.MapTypeId.ROADMAP,
        gestureHandling : 'cooperative'
      });
      this.layers[0] = new google.maps.KmlLayer({url:'https://www.temucotemueve.cl/ttm/servicio_mapa/kmz/colectivos/L17/17A_ida.kml', suppressInfoWindows: true, map: this.map2});
      this.layers[1] = new google.maps.KmlLayer({url:'https://www.temucotemueve.cl/ttm/servicio_mapa/kmz/colectivos/L17/17_ida.kml', suppressInfoWindows: true, map: this.map2});
      this.layers[2] = new google.maps.KmlLayer({url:'https://www.temucotemueve.cl/ttm/servicio_mapa/kmz/colectivos/L17/17A_retorno.kml', suppressInfoWindows: true, map: this.map2});
      this.layers[3] = new google.maps.KmlLayer({url:'https://www.temucotemueve.cl/ttm/servicio_mapa/kmz/colectivos/L17/17_retorno.kml', suppressInfoWindows: true, map: this.map2});
      for (var i = 0; i < this.layers.length; i++) {
        this.layers[i].setMap(null);
	    }

      //Creación del segundo marcador con texto 
      const secondMarkerLatLng = new google.maps.LatLng(-38.7, -72.6);
      ...
      this.secondMarker.addListener('click', () => {
        secondMarkerInfoWindow.open(this.map2, this.secondMarker);
      });

      this.moverMarcador(this.secondMarker, myLatLng, secondMarkerLatLng, 5000);
    }else{
      console.log("Error")
    }
  ...
  async ubicacionActual() {
    ...
    if(permission.location=='granted'){
      ...
      this.map2.setCenter(this.miPos);
      ...
      this.myMarker.addListener('click', () => {
        infoWindow.open(this.map2, this.myMarker);
      });
  }else{
    this.mostrarAlerta('Para utilizar esta función, necesitas habilitar los permisos de ubicación.');
  }
  ...
  toggleLayer(i: number) {
    ...
    if (this.layers[i].getMap() === null) {
      this.layers[i].setMap(this.map2);
    }
    else {
      this.layers[i].setMap(null);
    }
